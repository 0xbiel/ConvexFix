...
contract ConvexStakingWrapper is ERC20, IERC4626, ReentrancyGuard {
    ...
    constructor(
        ...
        address _cvx,
        address _factory
    ) ERC20("StakedConvexToken", "stkCvx") {
    ...
    function initialize(uint256 _poolId) external virtual {
        ...
        (address _lptoken, , address _rewards, , ) = IBooster(convexBooster)
            .poolInfo(_poolId);
        curveToken = _lptoken;
        convexPool = _rewards;
        ...
        _tokenname = string(
            abi.encodePacked("Wrapped ", ERC20(_rewards).name())
        );
    ...
    function addRewards() public {
        ...
        for (uint256 i = 0; i < rewardCount; i++) {
            ...
            IConvexRewardPool.RewardType memory rewardType = IConvexRewardPool(
                mainPool
            ).rewards(i);
            ...
            if (_isClaim || userI < reward.reward_integral) {
                if (_isClaim) {
                    uint256 receiveable = claimable_reward[reward.reward_token][
                        ...
                        ((_balances[u] *
                            uint256(reward.reward_integral - userI)) / 1e20);
                    if (receiveable > 0) {
                    ...
                    claimable_reward[reward.reward_token][_accounts[u]] =
                        claimable_reward[reward.reward_token][_accounts[u]] +
                        ((_balances[u] *
                            uint256(reward.reward_integral - userI)) / 1e20);
                }
                reward_integral_for[reward.reward_token][_accounts[u]] = reward
                    .reward_integral;
            }
        }
    ...
    function deposit(uint256 _amount, address _to) public returns (uint256) {
        ...
        if (_amount > 0) {
            _mint(_to, _amount);
            IERC20(curveToken).safeTransferFrom(
                msg.sender,
                address(this),
                _amount
            );
    ...
    function convertToSharesRoundUp(
        uint256 _assets
    ) internal pure returns (uint256) {
        return convertToShares(_assets);
    }

    function convertToAssetsRoundUp(
        uint256 _shares
    ) internal pure returns (uint256) {
