...
contract ConvexRewardPool is ERC20, ReentrancyGuard{
    ...
    function _insertRewardToken(address _token) internal{
        ...
        (bool success, bytes memory data) = _token.call(abi.encodeWithSelector(IERC20(_token).transfer.selector, address(this), 0));
        ...
        if(rewardMap[_token] == 0){
            ...
            //with new tokens
            (bool _success, bytes memory _data) = _token.call(abi.encodeWithSelector(IERC20(_token).transfer.selector, address(this), 0));
